...
var instrucciones = ["MUEVE LA PIEZA VACIA HASTA ORDENAR EL ROMPECABEZAS",
                     "EL MISMO DEBERA QUEDAR COMO SE INDICA EN EL OBJETIVO!",
                     "LA FLECHA TE INDICARA TU ULTIMO MOVIMIENTO",
                     "TIENE UNA CANTIDAD DE MOVIMIENTOS DETERMINADOS"];
...
//arreglo para intercambiar posiciones en la grilla
var posicionAux = [0, 0];

//Guardamos los carteles en variables para ser utilizada por las funciones
var ganador = document.getElementById("cartelGanador");
var perdedor = document.getElementById("cartelPerdedor");
var aceptar = document.getElementById("aceptar");
var intentar = document.getElementById("intentar");
var cartelMovimientos = document.getElementById("movimientos-restantes");
var movTotales = 40;
    ...
    for(var i=0; i< grilla.length;i++){
      for(var j=0; j< grilla[i].length;j++){
          if(grilla[i][j] != grillaGanadora[i][j]){
            ...
            if(i == grilla.length-1  && j== grilla[i].length-1){
              return true;
            }
          }
      }
...
/* Esta funcion chequea si la persona perdio, el algoritmo de mezcla utiliza 30 movimientos.
Por lo que el usuario deberia ser capaz de armar el rompecabezas entre 30 y 40 movimientos. 
*/
...
// Funcion para mostrar el cartel Perdedor
function mostrarCartelPerdedor() {
  if(chequearSiPerdio()== true){
    perdedor.style.display="block";
    intentar.style.display = "block";
  }
}
// Funcion para ocultar el Cartel de Ganador y Perdedor
function botonDeAceptar() {
...
//Funcion para mostrar el cartel del contador
function MostrarCantMov(){
  cartelMovimientos.textContent = "Los movimientos Restantes son: "+movTotales;
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    posicionAux = grilla[filaPos1][columnaPos1];
    for(var i=0; i< grilla.length;i++){
      for(var j=0; j< grilla[i].length;j++){
          if(i == filaPos1 && j == columnaPos1){
            grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
            grilla[filaPos2][columnaPos2] = posicionAux;
          }
      }
    }
}
...
function posicionValida(fila, columna) {
  if((fila >= 0) && (fila <= grilla.length-1)){
    if((columna >= 0) && (columna <= grilla.length-1)){
        return true;
  }else{
        return false;
    }
